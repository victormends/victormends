...
- **Eliminated the WAL retention risk class** across the fleet by building a Task Scheduler PowerShell monitor (PG12) that detects inactive orphaned replication slots and applies threshold-gated auto-drop (2GB retained / 30 min inactive), with Windows Event Log alerts; on PG13+ instances, complemented with `max_slot_wal_keep_size` as a hard cap.
...
- **Engineered a 70M-record ETL pipeline** cutting processing time 60% on a 15GB+ government dataset; extended with client classification (MEI, Simples Nacional, Normal, invalid CNPJ), XML regime auditing across NF-e/NFC-e/NFS-e/CT-e, and PostgreSQL staging, surfacing the top 0.001% of leads for commercial prioritization.
